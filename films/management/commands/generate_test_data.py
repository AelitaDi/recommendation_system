...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        producer_list = [
            Producer(name=f"Режиссер {i}", bio=f"Режиссер {i} Биография")
        ...
        actor_list = [
            Producer(name=f"Актер {i}", bio=f"Актер {i} Биография")
        ...
        for i in range(1, num_users + 1):
            ...
            user.is_active = True
            user.preferred_genres.add(
                *random.sample(list(genres), k=random.randint(2, 4))
            )
            user.save()
        ...
        for user in all_users:
            ...
            preferred_films = Film.objects.filter(
                genres__in=user_preferred_genres
            ).distinct()
            other_films = Film.objects.exclude(
                genres__in=user_preferred_genres
            ).distinct()
            ...
            for film in preferred_sample + other_sample:
                rating = random.choices(
                    [None, round(random.randint(3, 10), 1)], weights=[0.3, 0.7]
                )[0]
        ...
        for film in films_to_update:
            film.average_rating = (
                Interaction.objects.filter(film=film).aggregate(Avg("rating"))[
                    "rating__avg"
                ]
                or 0.0
            )
