...
class KNN:
    ...
    def recommendations(user_id, top_n=10):
        ...
        """
        ranked_films = list(
            Interaction.objects.filter(user=user_id)
            .exclude(rating=None)
        ...
        neighbours = list(
            Interaction.objects.filter(film_id__in=ranked_films, rating__isnull=False)
            .distinct()
        ...
        for nei in neighbours:
            ...
            for film in ranked_films:
                ...
                if (
                    Interaction.objects.filter(user=nei, film=film)
                    .exclude(rating=None)
                    .exists()
                ):
        ...
        top_films = (
            Interaction.objects.filter(user_id__in=sorted_nei[:k], rating__gte=4)
            .exclude(film_id__in=ranked_films)
            .values_list("film_id", flat=True)
            .distinct()
        )
        ...
        for f in top_films:
            rec_films.append(
                {"film_id": f, "av_r": Film.objects.get(id=f).average_rating}
            )

        sorted_top_n_rec_films = sorted(
            rec_films, key=lambda x: x["av_r"], reverse=True
        )[:top_n]
